  ...
  <div>
      ...
      <v-list class="w-11/12 mx-auto space-y-2">
        ...
        <v-list-item class="flex flex-row items-center justify-between">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
              ...
              ></v-tooltip>
              <v-tooltip
                :text="showWorkspaces ? 'hide workspace' : 'show workspaces'"
                :open-delay="1000"
              >
                <template v-slot:activator="{ props }">
                  <v-btn
                    ...
                    variant="text"
                    size="xx-small"
                    @click="() => toggleShowWorkspaces()"
                  >
                    <Icon
                      :icon="showWorkspaces ? 'ph:caret-up' : 'ph:caret-down'"
                      width="20"
                      class="cursor-pointer"
                    />
                  </v-btn> </template
        ...
        <v-list class="items-start flex flex-col" v-show="showWorkspaces">
          <v-list-item-subtitle
            class="pb-5"
            v-if="myAdminWorkspaces && myAdminWorkspaces.length"
          >
            Your workspaces
          </v-list-item-subtitle>
          <v-list-item
            v-for="workspace in myAdminWorkspaces"
            :key="workspace"
            class="w-full items-start"
          >
            <WorkspaceListItem
              :workspace="workspace"
              @open-member-dialog="() => openAddMemberDialog(workspace)"
            />
          </v-list-item>
          <v-list-item-title
            v-if="myMemberWorkspaces && myMemberWorkspaces.length"
          >
          ...
          <v-list-item
            v-for="workspace in myMemberWorkspaces"
            :key="workspace"
            class="w-full items-start"
          >
            <WorkspaceListItem
    ...
    <v-main v-if="authStore.user">
      ...
      <v-alert
        ...
        border="start"
        density="compact"
        class="w-[400px] !fixed bottom-2 right-2"
        text="Your email is not verified you cannot be added to any workspace. please verify it from settings"
      ></v-alert>
      <!-- </v-container> -->
