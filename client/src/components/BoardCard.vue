...
const isFavorite = computed(() =>
  favoriteBoards.value.some(
    (favoriteBoard) => favoriteBoard.id === props.board.id
  )
);
</script>
...
<template>
  <div
    ...
    @mouseover="showFavBtn = true"
    @mouseleave="showFavBtn = false"
  >
    <router-link :to="{ path: `/b/${board.id}` }" class="group">
      <v-card
        class="cursor-pointer"
        :class="
          board.backgroundColor == 'default'
            ? '!bg-gray-300 dark:!bg-darkPrimary cursor-pointer'
            : ''
        "
        min-height="120"
        :color="
          board.backgroundColor === 'default' ? '' : board.backgroundColor
        "
      >
        <v-card-title>
          {{ board.name }}
    ...
    >
      <Transition name="slide-fade">
        <v-btn
          ...
          variant="text"
          size="x-small"
          class="mr-2 mb-2 right-0 group/fav z-50 absolute block"
          v-if="showFavBtn"
          :ripple="false"
        >
          <Icon
            ...
            width="15"
            class="group-hover/fav:scale-150 transition-all"
