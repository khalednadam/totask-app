...
<template>
  <v-menu v-model="dateMenu" :close-on-content-click="false" location="end">
    <template v-slot:activator="{ props }">
      <div v-if="isDateAsTitle">
        <v-btn v-bind="props" variant="tonal" :color="color" :key="color">
          <p>
            {{ new Date(endDate)?.toLocaleString("en-GB", fullDateOptions) || null }}
          </p>
        </v-btn>
      </div>
      <div v-else>
    ...
    <v-card>
      ...
      <VueDatePicker range inline :dark="theme.global.name.value === 'dark'" v-model="model">
        <template #action-row="{ internalModelValue, selectDate }">
          <div class="action-row flex gap-1 w-full">
            <v-btn class="select-button w-1/2" color="primary" variant="outlined" @click="$emit('onDeleteDate')">
              Delete
            </v-btn>
            <v-btn class="select-button w-1/2" color="primary"
              @click="() => { selectDate(); $emit('updateCardDates'); dateMenu = false; }">Select
              Date</v-btn>
          </div>
        </template>
      </VueDatePicker>
