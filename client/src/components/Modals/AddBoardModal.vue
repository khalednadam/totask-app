...
<template>
  <v-card
    scrollable
    class="self-center h-full md:w-2/3 overflow-y-auto"
    rounded="lg"
  >
    <v-card-title>
    ...
    <v-row>
      <v-col cols="12" lg="6">
        <v-card-text class="">
          ...
          <v-select
            v-model="privateMembers"
            multiple
            v-if="isPrivate && workspace"
          ...
          <v-select
            :key="chosenWorkspace"
            v-model="chosenWorkspacePrivateMembers"
            multiple
            v-if="isPrivate && !workspace"
            :items="chosenWorkspace?.members"
            label="members"
            item-title="name"
            item-value="id"
          >
      ...
      <v-col v-if="lgAndUp" cols="12" md="6" class="">
        <v-row
          class="justify-center items-center h-full my-auto mx-auto w-full"
        >
          <v-col cols="1"></v-col>
          <v-col
            cols="10"
            class="h-[50%] rounded-lg max-h-[220px]"
            :style="
              boardBackgroundColor !== 'default'
          ...
          >
            <v-row class="h-full my-auto mx-auto space-x-3">
              <v-col
                class="h-full my-auto rounded-lg"
                :style="{ backgroundColor: 'rgb(var(--v-theme-list))' }"
              >
                <v-row>
                      ...
                      <v-card-text>
                        <v-row>
                          <v-col cols="12" class="space-y-2">
                            <div
                              class="w-1/2 h-1 bg-gray-400 rounded-full"
                            ></div>
                            <div
                              class="w-full h-1 bg-gray-400 rounded-full opacity-40"
                            ></div>
                          </v-col>
                        </v-row>
                      </v-card-text>
