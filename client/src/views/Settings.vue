...
const deleteAccount = () => {
  ...
  axiosInstance.post(`/users/${user.value.id}`, {
...
onMounted(async () => {
  ...
  resetForm({ values: { name: user.value.name, username: user.value.username, email: user.value.email } })
})

const deleteProfilePic = handleSubmit(() => {
  isLoading.value = true
  axiosInstance.put(`/users/deleteProfilePic/${user.value.id}`, {
    file: null
  },
    {
      headers: {
...
<template>
  ...
  <div class="mt-5">
    ...
    <p class="text-xl text-error mt-5">
      Delete account
    </p>
    <v-divider class="mt-2 border-2 border-black dark:border-white"></v-divider>
  ...
  <v-dialog v-model="deleteAvatarDialog" class="md:max-w-[50vw] w-full">
    <DeleteModal :is-loading="isLoading" title="Delete Profile Picture" actionBtnText="Delete"
      text="Your profile picture will be deleted" @cancel="deleteAvatarDialog = false"
      @delete="() => { profilePic = null; deleteProfilePic() }" />
  </v-dialog>
