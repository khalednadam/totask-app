...
import { useBoards, getWorkspace, useCloasedBoards } from "../composables/utils";
...
const { closedBoards, closedBoardsLoading } = useCloasedBoards(route.params.workspaceId);
...
const deleteBoard = (boardId) => {
  ...
  axiosInstance.delete(`/b/${boardId}`,
...
<template>
      ...
      <v-card-text>
        <v-data-table :headers="headers" :items="closedBoards">
          ...
          <template v-slot:item.actions="{ item }">
            <div class="space-x-2">
              <v-btn color="error" variant="flat" @click="() => selectBoardToDelete(item)">
                <Icon icon="ph:x" />
                Delete
              </v-btn>
              <v-btn @click="() => reopenBoard(item)" color="primary" variant="flat">
                Reopen
              </v-btn>
            </div>
          </template>
  ...
  <DangerDeleteModal :name="boardToDelete?.name" :isLoading="closedBoardsLoading" v-model="deleteDialog"
