...
const updatePost = async () => {
  isLoading.value = true;
  try {
    const response = await axiosInstance.put(`/blog/${blogPostId.value}`, {
...
<template>
  <div class="my-5">
    ...
    <div v-else>
      ...
      <QuillEditor ref="quill" toolbar="essential" v-model:content="postText" @text-change="setText" contentType="html"
        theme="snow" />
      <div class="mt-5 gap-2 flex justify-end">
        <v-btn variant="outlined" color="primary" @click="goBack">
          Cancel
        </v-btn>
        <v-btn @click="updatePost" flat color="primary" :disabled="isLoading" :loading="isLoading">
          Update
        </v-btn>
      </div>
    </div>
